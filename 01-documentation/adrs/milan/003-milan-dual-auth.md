...
However, relying _solely_ on Google OAuth is a single point of failure and creates friction for users purchasing tickets on behalf of others (or using non-Google emails). By building a custom OTP system alongside it, we ensure that anyone can access the platform without ever maintaining a password database.
